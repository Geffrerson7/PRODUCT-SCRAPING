...
# Crear DataFrames de pandas con los enlaces de imágenes y los SKU
df_enlaces_imagenes = pd.DataFrame(enlaces_imagenes_total, columns=['Enlace de la Imagen'])
df_sku_productos = pd.DataFrame(sku_productos_total, columns=['SKU'])

# Concatenar ambos DataFrames
df_final = pd.concat([df_enlaces_imagenes, df_sku_productos], axis=1)

# Guardar el DataFrame en un archivo Excel
df_final.to_excel('enlaces_imagenes_y_sku_2.xlsx', index=False)

print("Los enlaces de las imágenes y los SKU se han guardado en 'enlaces_imagenes_y_sku.xlsx'")
